...
const mongoUrl = process.env.MONGODB_URI || 'mongodb://localhost/hacklord';
const defaultPassword = process.env.HACKERLOG_PASSWORD || 'stephen';
...
app.get('/', (req, res) => {
  ...
  Update.find().limit(perPage).skip(perPage * page).sort({ createdAt: 'desc' }).exec().then((updates) => {
    Update.count().exec().then((count) => {
      ...
      res.render('index', { title: 'Stephen', updates, wrongPassword, page, pages, moment });
    }).catch(() => {
      res.redirect('/error');
    });
  }).catch(() => {
